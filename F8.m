% % Averaged biases of H_SEP_n and H_EEP_n between models and observation with global mean SSM for observation over different time scales
...
for k=1:account
    ...
    spdif_comp1{k}=avg_mhpdif1(idx{k});
...
for k=1:account
    spdif_comp2{k}=avg_mhpdif2(idx{k});
...
for k=1:account
    spdif_comp3{k}=avg_mhpdif3(idx{k});
...
for k=1:account
    hpdif_comp1{k}=avg_hhpdif1(idx{k});
...
for k=1:account
    hpdif_comp2{k}=avg_hhpdif2(idx{k});
...
for k=1:account
    hpdif_comp3{k}=avg_hhpdif3(idx{k});
...
h=text(-0.05,0.5,'Normalized global mean SSM (SMAP)');
set(h,'fontsize',11,'VerticalAlignment','middle')
axis([plot1 plot3],[0 1 -1 1])
axis(plot2,[0 1 -4 4])
axis([plot4 plot5 plot6],[0 1 -0.4 0.4])